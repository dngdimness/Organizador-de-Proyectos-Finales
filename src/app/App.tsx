...
import { calculateCategoryScore } from './utils/category-scoring';
...
export default function App() {
  ...
  // Guardar automáticamente en localStorage cuando cambian los items o el título
  useEffect(() => {
    if (projectItems.length > 0 || projectTitle) {
      const data = { items: projectItems, title: projectTitle };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    }
  }, [projectItems, projectTitle]);

  // Splash screen, favicon y logo dinámico
  useEffect(() => {
    // Splash screen timer
    ...
    // Configurar favicon (siempre modo claro)
    const setFavicon = () => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    // Para otros componentes, verificar si ya existe
        ...
        <div className="max-w-screen-2xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowAboutDialog(true)} aria-label="Acerca de">
          ...
          <div className="mt-4">
            <BudgetBar 
              ...
              coveredCategoriesCount={coveredCategories.size}
                    ...
                    {projectItems.map((item) => {
                      ...
                      return (
                        <ProjectItem
                          ...
                          hasValidationError={invalidItemIds.includes(item.id)}
                        />
                      );
                    })}
                  </div>
